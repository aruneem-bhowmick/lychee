...
from lychee.config import LycheeConfig, ReviewConfig, ScopeRule
...
# ---------------------------------------------------------------------------
# Integration tests -- scope-rule file filtering in build_context()
# ---------------------------------------------------------------------------


def test_build_context_ignores_scoped_files() -> None:
    """build_context() with scope rules filters out files matching ignore rules."""
    mock_client = MagicMock()
    mock_pr = _build_mock_pr()
    mock_client.get_pull_request.return_value = mock_pr
    mock_client.get_diff.return_value = "diff"
    mock_client.get_changed_files.return_value = [
        ChangedFile("src/app.py", "modified", 5, 2, "+code", "code"),
        ChangedFile("generated/output.py", "added", 10, 0, "+gen", "gen"),
        ChangedFile("src/core.py", "modified", 3, 1, "+core", "core"),
    ]
    mock_client.get_commit_messages.return_value = []
    mock_client.get_conventions_file.return_value = None
    ...
    ctx = build_context(mock_client, ref, config, pr_labels=[])

    assert len(ctx.changed_files) == 2
    filenames = [f["filename"] for f in ctx.changed_files]
    assert "src/app.py" in filenames
    assert "src/core.py" in filenames
    assert "generated/output.py" not in filenames


def test_build_context_scope_rules_with_labels() -> None:
    """build_context() scope rules respect pr_labels for filtering."""
    mock_client = MagicMock()
    mock_pr = _build_mock_pr()
    mock_client.get_pull_request.return_value = mock_pr
    mock_client.get_diff.return_value = ""
    mock_client.get_changed_files.return_value = [
        ChangedFile("src/app.py", "modified", 5, 2, "+code", "code"),
        ChangedFile("src/auto.py", "added", 10, 0, "+gen", "gen"),
    ]
    mock_client.get_commit_messages.return_value = []
    mock_client.get_conventions_file.return_value = None

    config = LycheeConfig(
        review=ReviewConfig(scope_rules=[ScopeRule(labels=["autogenerated"], ignore=True)])
    )
    ref = PullRequestRef.parse("owner/repo#42")

    # Without the label, no files are filtered
    ctx_no_label = build_context(mock_client, ref, config, pr_labels=[])
    assert len(ctx_no_label.changed_files) == 2

    # With the label, all files are filtered (empty paths = match all)
    ctx_with_label = build_context(mock_client, ref, config, pr_labels=["autogenerated"])
    assert len(ctx_with_label.changed_files) == 0


def test_build_context_no_scope_rules_unchanged() -> None:
    """build_context() without scope rules produces the same output as before."""
    mock_client = MagicMock()
    mock_pr = _build_mock_pr()
    mock_client.get_pull_request.return_value = mock_pr
    mock_client.get_diff.return_value = "diff"
    ...
    mock_client.get_commit_messages.return_value = ["init"]
    mock_client.get_conventions_file.return_value = None

    config = LycheeConfig()  # No scope_rules
    ref = PullRequestRef.parse("owner/repo#42")

    ctx = build_context(mock_client, ref, config)
    assert len(ctx.changed_files) == 1
    assert ctx.changed_files[0]["filename"] == "src/app.py"


def test_build_context_pr_labels_default_none() -> None:
    """build_context() works without pr_labels (backward compatible)."""
    mock_client = MagicMock()
    mock_pr = _build_mock_pr()
    mock_client.get_pull_request.return_value = mock_pr
    mock_client.get_diff.return_value = ""
    ...
    config = LycheeConfig(
        review=ReviewConfig(scope_rules=[ScopeRule(labels=["skip"], ignore=True)])
    )
    ref = PullRequestRef.parse("owner/repo#42")

    # pr_labels defaults to None, so label-based rules don't match
    ctx = build_context(mock_client, ref, config)
    assert len(ctx.changed_files) == 1
